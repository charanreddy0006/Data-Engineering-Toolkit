...
ETL transforms data before loading it into the final destination. ELT loads source data first and performs transformations inside the analytical platform.
...
ETL can reduce the amount of data entering a destination and can enforce preprocessing early. ELT keeps raw information available and takes advantage of scalable warehouse or lakehouse compute.
...
ETL is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### ELT

ELT is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Staging

Staging is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Raw layer

Raw layer is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Curated layer

Curated layer is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Warehouse compute

Warehouse compute is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.
...
```text
ETL: Source -> Extract -> Transform -> Load
ELT: Source -> Extract -> Load -> Transform
```
...
Automobile example: sensitive service information can be filtered during ETL, while large telemetry datasets can be loaded raw and transformed later using Spark or SQL.
...
Choose based on data volume, destination capabilities, governance, latency, cost and reprocessing requirements.
