...
A full load copies the complete source dataset. An incremental load processes only changes.
...
Full loads are easier to reason about but become expensive as history grows. Incremental loads are efficient but require reliable change detection.
...
Initial load is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Incremental load

Incremental load is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Backfill

Backfill is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Watermark

Watermark is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Reconciliation

Reconciliation is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.
...
```text
Initial Full Load -> Incremental Loads -> Periodic Reconciliation
```
...
Automobile example: load five years of service history once and then ingest daily changes.
...
Many production systems combine incremental processing with occasional reconciliation or snapshot jobs.
