...
Recovery allows a pipeline to continue after transient failures while avoiding corrupted output.
...
Use bounded retries, exponential backoff and checkpoints. Permanent failures should eventually surface as alerts.
...
Transient failure is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Backoff

Backoff is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Retry limit

Retry limit is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Checkpoint

Checkpoint is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Replay

Replay is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Recovery

Recovery is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.
...
```text
Task -> Failure -> Backoff -> Retry -> Success / Escalate
```
...
Automobile example: retry a temporary cloud-storage timeout three times before marking the task failed.
...
Recovery design should be tested, not assumed. A pipeline that cannot recover safely is operationally fragile.
