...
Incremental loading processes only records that are new or changed since a known position.
...
Common mechanisms are timestamps, monotonically increasing IDs, sequence numbers and CDC offsets.
...
High-water mark is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Updated timestamp

Updated timestamp is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Sequence ID

Sequence ID is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.
...
```text
Last Watermark -> Source Query -> Changed Rows -> Target -> New Watermark
```
...
Automobile example: process service records whose `updated_at` value is greater than the last successful watermark.
...
Advance the watermark only after the target write and validation have succeeded.
