...
An idempotent pipeline produces the same correct final state when the same input is processed more than once.
...
Idempotency protects against retries, worker restarts and accidental duplicate execution.
...
Deterministic keys is an important part of this concept. Understand its responsibility,
...
Checkpoints is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Exactly-once effect

Exactly-once effect is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.
...
```text
Input -> Process -> Target State
Same Input Again -> Same Target State
```
...
Automobile example: reprocessing event E100 must not create two copies of the same event.
...
Idempotency is one of the most important properties of reliable data pipelines.
