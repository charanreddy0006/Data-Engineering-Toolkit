...
Loading writes processed data into the destination system.
...
Loading strategies include append, overwrite, merge and upsert. The correct strategy depends on whether records are immutable, mutable or periodically rebuilt.
...
Append is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Overwrite

Overwrite is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Upsert

Upsert is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.
...
Bulk loading is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Partitioned loading

Partitioned loading is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.
...
```text
Processed Data -> Load Job -> Target Table / Files
```
...
Automobile example: upsert changed service records and append immutable telemetry events.
...
Load operations should be observable and should expose row counts, rejected records and target freshness.
