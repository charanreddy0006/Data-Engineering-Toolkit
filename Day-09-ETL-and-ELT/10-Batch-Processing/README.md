...
Batch processing groups records and processes them on a schedule or trigger.
...
Batch jobs are appropriate for workloads where seconds of latency are not required. They can be simpler and cheaper than always-on streaming systems.
...
Scheduled jobs is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Daily loads

Daily loads is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Hourly loads

Hourly loads is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Backfills

Backfills is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Windowed processing

Windowed processing is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.
...
```text
Schedule -> Extract Window -> Transform -> Load -> Validate
```
...
Automobile example: generate a daily warranty-cost summary for every vehicle model.
...
Batch systems should still support retries, idempotency, logging and late-arriving data.
