...
Monitoring tells engineers whether pipelines are running correctly and whether the produced data is healthy.
...
Observe both infrastructure metrics and data metrics.
...
Job status is an important part of this concept. Understand its responsibility,
...
Freshness is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Volume

Volume is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Failure rate

Failure rate is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.
...
```text
Pipeline -> Metrics + Logs -> Dashboard -> Alert
```
...
Automobile example: alert when hourly telemetry volume drops by 50 percent.
...
A green job status alone is not enough. A job can succeed while producing zero or stale records.
