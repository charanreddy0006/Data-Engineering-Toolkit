...
Pipelines need a clear strategy for malformed records, source failures, timeouts and destination errors.
...
Data errors and infrastructure errors should not be treated identically. Bad records may be quarantined while transient failures may be retried.
...
Quarantine is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Retry

Retry is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Dead-letter queue

Dead-letter queue is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Alerts

Alerts is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.

### Failure logs

Failure logs is an important part of this concept. Understand its responsibility,
inputs, outputs and failure conditions before implementing it in a production
pipeline.
...
```text
Failure -> Classify -> Retry / Quarantine / Stop / Alert
```
...
Automobile example: malformed sensor data goes to quarantine while a temporary database timeout is retried.
...
Every error path should preserve enough context to diagnose and replay the failure.
